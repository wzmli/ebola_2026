...
ggdt <- (ggplot(dflist, aes(date,group=province))
	...
	+ facet_wrap(~province,ncol=3,scale="free")
#	+ geom_pointrange(data=phydat,aes(x=date,y=med,ymin=lwr,ymax=upr))
	+ xlim(c(as.Date("2026-06-01"),as.Date("2026-08-27")))
...
ggcases <- (ggplot(filter(dat,new_cases>0, province %in% c("Ituri","Nord-Kivu","Haut-Uele")), aes(date))
